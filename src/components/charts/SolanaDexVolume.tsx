...
export function SolanaDexVolume({
  ...
  const groupedData = DexVolumeData.result.rows.reduce(
    (acc: GroupedDataItem[], { amount_usd, project, time }) => {
      ...
      if (existingMonthYear) {
        existingMonthYear[project] =
          ((existingMonthYear[project] as number) || 0) + amount_usd;
      } else {
  ...
  return (
    <Card>
      <CardHeader>
        <CardDescription>Solana DEX Volumes in USD</CardDescription>
      </CardHeader>
      <CardContent>
